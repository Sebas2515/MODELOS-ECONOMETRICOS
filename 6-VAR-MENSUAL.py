...
path = Path('DATA/base_tesis_mensual.xlsx')

# Cargar la hoja específica para la tesis
df = pd.read_excel(path, sheet_name='base_mensual')

# Convertir la columna 'Año' al formato datetime (día.mes.año)
df['Año'] = pd.to_datetime(df['Año'], format='%d.%m.%Y')

df.set_index(pd.PeriodIndex(df['Año'], freq='M'), inplace=True)

# (Opcional) mantener el nombre del índice como 'Año'
df.index.name = 'Año'
...
non_stationary_vars = []  # lista para guardar las no estacionarias
for name, col in df_n_diff.items():
    ...
    if result[1] <= 0.05:
        ...
        print("❌ Serie no estacionaria (tiene raíz unitaria).")
        non_stationary_vars.append(name)

################################################################################
# PASO 3.1: APLICAR SEGUNDA DIFERENCIA SOLO A N_IPC y N_TIR (y verificar ADF)
################################################################################
# Hacemos una copia de df_n_diff para construir el dataset que irá al VAR
df_var = df_n_diff.copy()

# Aplicar segunda diferencia SOLO a N_IPC y N_TIR
to_second_diff = ['N_IPC', 'N_TIR']
for var in to_second_diff:
    df_var[var] = df_n_diff[var].diff()   # esto es la segunda diferencia respecto al nivel original

# Eliminar filas nulas generadas por las diferencias
df_var = df_var.dropna()

# Verificar ADF para las series transformadas (asegurar estacionariedad)
print("\n--- 4.1 ADF en las series después de aplicar 2ª diferencia a IPC y TIR ---")
for var in to_second_diff:
    adf_test(df_var[var], var + "_(2da_diff)")

# (Opcional) verificar ADF para las demás ya en primera diferencia (por control)
print("\n--- 4.2 ADF control: otras series en 1ª diferencia ---")
for col in df_var.columns:
    if col not in to_second_diff:
        adf_test(df_var[col], col + "_(1ra_diff)")
